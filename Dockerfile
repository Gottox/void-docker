...
ARG repository=https://alpha.de.repo.voidlinux.org/current/musl
...
RUN apk add ca-certificates && \
  wget -O - https://alpha.de.repo.voidlinux.org/static/xbps-static-latest.$(uname -m)-musl.tar.xz | \
    tar Jx && \
  XBPS_ARCH=$(uname -m)-musl xbps-install.static -yMU \
...
RUN xbps-reconfigure -a && \
  ...
  XBPS_ARCH=$(uname -m)-musl xbps-install -yMU \
    --repository=$repository \
    -r /stage2 \
    base-minimal
